...
logger<<"start mknod "<<name<<parent;
  FileOperation *fs = reinterpret_cast<FileOperation *>(fuse_req_userdata(req));
...
void chfs_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name,
                mode_t mode) {
logger<<"start mkdir "<<name<<parent;
  struct fuse_entry_param e;
...
logger<<"start unlink "<<name<<parent;
  FileOperation *fs = reinterpret_cast<FileOperation *>(fuse_req_userdata(req));
...
logger<<"start lookup"<<name<<parent;
  FileOperation *fs = reinterpret_cast<FileOperation *>(fuse_req_userdata(req));
  struct fuse_entry_param e;
...

  {
    ...
    if (res.is_err()) {
      ...
      fuse_reply_err(req, -1);
logger<<"lookup cant find directory";
      return;
    }
  }
for (auto it = list.begin(); it != list.end(); ++it) 
logger<<"list item id:"<<(*it).id<<" name:"<<(*it).name<<"\n";
  ...
  for (const auto &entry : list) {
    if (entry.name == name) {
      ...
      // get attr
      {
        ...
        if (attr_res.is_err()) {
          fuse_reply_err(req, -1);
logger<<"lookup cant get type attr";
          return;
        }
      ...
      fuse_reply_entry(req, &e);
logger<<"success lookup"<<e.ino;
      return;
    }
  }

logger<<"lookup enoent";
  fuse_reply_err(req, ENOENT);
}
