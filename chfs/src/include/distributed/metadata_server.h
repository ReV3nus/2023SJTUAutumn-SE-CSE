...
public:
  ...
  auto recover() -> void {
    ...
    commit_log->recover();
    if(commit_log->recover_operations.size())
    {
      for(const auto &operation : commit_log->recover_operations)
      {
        if(!operation.type)
        {
          unlink(operation.parent, operation.name);
        }
        else
        {
          mknode(operation.type, operation.parent, operation.name);
        }
      }
    }
