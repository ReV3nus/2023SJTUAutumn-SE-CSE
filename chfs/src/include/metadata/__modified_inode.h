...
class WrappedBlockInfo {
public:
    operator block_id_t() const {
        return std::get<0>(info);
    }
    WrappedBlockInfo(block_id_t value) : info(std::make_tuple(value, 0, 0)) {}
    WrappedBlockInfo():info(std::make_tuple(0,0,0)){}
...
public:
  [[maybe_unused]] WrappedBlockInfo blocks[0];
...
public:
  ...
  Inode(InodeType type, usize block_size)
    ...
    nblocks = (block_size - sizeof(Inode)) / sizeof(WrappedBlockInfo);
  ...
  block_id_t &operator[](size_t index) {
    ...
    return std::get<0>(this->blocks[index].info);
  }
...
public:
  ...
  block_id_t &operator*() const { return std::get<0>(inode->blocks[cur_idx].info); }
};
